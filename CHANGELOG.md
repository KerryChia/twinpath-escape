...
- Added an AI oscillation breaker: three steering-direction reversals inside a 1.2s window and a ~56px radius count as the in-place left/right shake; the controller then releases all keys for 0.35s, books the edge as a failure (blacklisted after two) and replans, so the loop cannot continue.
- Removed the per-frame simulation storm behind the twitch stutter: failed rollout sweeps schedule a 0.5s cooldown that replays their best-effort steering, the sticky script cache key is quantized to a 6px grid, and TMXMap now caches scaled layer rects (invalidated on rescale) instead of re-scaling hundreds of tiles on every physics/AI access. Full-campaign headless time drops 12.2s to 8.6s and the dual-AI completion regression suite itself falls from ~985s to ~16s.
